...
/** cloud connected event callback 云连接成功 回调事件 */
...
/** cloud connect fail event callback 云连接失败 回调事件 */
...
/** cloud disconnected event callback 云连接断开 回调世界 */
...
/* device initialized event callback 设备初始化事件 */
...
/** recv property post response message from cloud 接收到云端对property的应答**/
...
/** recv event post response message from cloud 接收到云端对触发事件的应答**/
...
{
    int res = 0;
    cJSON *root = NULL, *LightState = NULL;
    ...
    LightState = cJSON_GetObjectItem(root, "powerstate");
    if (LightState) {
        //lightbulb_status_set(LightState->valueint);
        if(LightState->valueint)
			SmartLED_Color_Set(LC_WHITE);
		else
			SmartLED_Color_Set(LC_BLACK);
    } 

	cJSON *Light_Color = cJSON_GetObjectItem(root, "color");
	if(Light_Color != NULL)
	{
		SmartLED_Color_Parse(Light_Color->valueint);
	}
...
static int linkkit_thread(void *paras)
{
    ...
    /* Create Master Device Resources 创建(初始化)主设备资源*/
    ...
    /* Start Connect Aliyun Server 连接阿里云服务器 */
